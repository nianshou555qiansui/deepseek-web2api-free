...
> 将 DeepSeek Chat (chat.deepseek.com) 的私有 API 转换为 OpenAI 兼容格式，支持流式输出、思维链/推理展示、工具调用（Function Calling），以及专家模式（深度推理，即 R1 风格 CoT）和联网搜索。
...
- **环境变量控制** — `MODE`/`THINKING` 环境变量独立控制模式选择，支持强制覆盖
...
这两个环境变量是独立的控制维度，共同决定每个请求的行为：

| 环境变量 | 可选值 | 默认值 | 说明 |
|----------|--------|--------|------|
| `MODE` | `auto` / `quick` / `expert` | `auto` | 控制 `model_type`：`null`(quick) 或 `"expert"` |
| `THINKING` | `auto` / `enabled` / `disabled` | `auto` | 控制 `thinking_enabled`：`true` / `false` |

优先级：环境变量 > 请求参数。当环境变量为 `auto` 时，`thinking_mode` 和默认逻辑决定行为。

### 四种组合

| MODE | THINKING | 行为 | 典型场景 |
|------|----------|------|----------|
| `auto` | `auto` | 由请求参数 `thinking_mode` 决定：`true`=expert+think，`false`=quick+no_think | 由客户端灵活控制 |
| `quick` | `disabled` | 强制 V3 快速模式，无推理 | 追求低延迟、不需要深度推理 |
| `expert` | `enabled` | 强制 R1 专家模式 + 思维链输出 | 需要深度推理和展示思考过程 |
| `expert` | `disabled` | 专家模式但无思维链（罕见组合） | 需要 expert 推理质量但不想暴露 CoT |

### 环境变量与请求参数互斥

```
MODE=auto, THINKING=auto, 请求 thinking_mode=true  →  expert + 有 reasoning_content
MODE=expert, THINKING=disabled, 请求 thinking_mode=true  →  expert + 无 reasoning_content
MODE=quick, THINKING=enabled, 请求 thinking_mode=false  →  quick + 有 reasoning_content
```
