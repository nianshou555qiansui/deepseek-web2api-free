...
- 预览版 (--pre)：`v2.1.0-pre` — 基础功能 + 工具调用 + 专家模式
...
| `start.bat` | Windows 一键启动脚本。自动检测并释放 8080 端口（通过 `netstat` + `taskkill`），然后启动 uvicorn。如果端口被其他进程占用，会先强制终止该进程及其子进程。 |
...
| `__wbindgen_export_0` | (len, align) → ptr | WASM 内存分配 |
...
当前 `_sessions` 字典**没有清理机制**：

```python
# server.py
_sessions: dict[str, str] = {}  # 只增不减
```

每次请求创建一个新 session（`_get_session()`），但从不删除。后果：
- 每次请求占用约 150 字节（UUID(36) + DS-SessionID(36) + dict 开销）
- 100 万次请求约 150MB 内存
- 长时间运行的服务会持续增长

**当前无影响的原因：**
- session-per-request 设计下，单个请求生命周期极短
- DeepSeek 服务端的 session TTL 约 3 天（259200s），即使 `_sessions` 记住旧 ID 也无害
- 2026 年的 Python dict 内存效率足够应付常规负载

**如需修复**，可添加 TTL 驱逐（如 `dict[str, tuple[str, float]]` 存时间戳），或改用 `cachetools.TTLCache`：

```python
from cachetools import TTLCache
# 3 小时自动过期
_sessions: dict[str, str] = TTLCache(maxsize=10000, ttl=10800)
```
...
## 7. MODE & THINKING 控制系统

### 7.1 设计思路

DeepSeek 的 API 有两个独立参数控制模型行为：
- **`model_type`**: `null`（快速模式）或 `"expert"`（专家模式）
- **`thinking_enabled`**: `true/false` — 是否输出推理过程

**关键认知：这两个参数完全独立。**

历史版本曾错误地将 `thinking_enabled=true` 等同于专家模式。实际 `model_type=null`（快速模式）+ `thinking_enabled=true` 也是有效组合——快速模式可以同时开启思考。

项目通过两个 `.env` 环境变量暴露这两个自由度：
...
`auto` 表示"由客户端 request 中的 `thinking_mode` 字段决定"，非 `auto` 值会覆盖客户端传参。
...
注意 `model_type` 只在 `MODE=expert` 时才设为 `"expert"`，其他情况（包括 `"auto"`）均为 `None`。这意味着 **MODE=auto + 客户端传 thinking_mode=true 不会自动切换到专家模式**——它仍然是快速模式（model_type=null）+ 开启思考（thinking_enabled=true）。

### 7.3 四种组合及 OpenAI SSE 行为

| MODE | THINKING | model_type | thinking_enabled | 实际效果 | reasoning_content | content |
|------|----------|------------|-----------------|---------|-------------------|---------|
| quick | disabled | `null` | `false` | 快速模式，无思考 | 无 | ✅ 有 |
| quick | enabled | `null` | `true` | 快速模式 + 思考 | ✅ 有 | ✅ 有 |
| expert | disabled | `"expert"` | `false` | 专家模式，无思考 | 无 | ✅ 有 |
| expert | enabled | `"expert"` | `true` | 专家模式 + 思考 | ✅ 有 | ✅ 有 |

所有 4 种组合均已通过原始 SSE 抓包验证。

**MODE=auto 时的回退行为：**
- MODE=auto → model_type=None（快速模式）
- THINKING=auto → thinking = req.thinking_mode or False
- 这意味着 `MODE=auto, THINKING=auto` 等价于旧版行为（但 model_type 不再与 thinking 绑定）

### 7.4 "快速模式 + 思考" 的 SSE 格式
...
### 7.7 状态机（`chat_stream` 中的 `frag_type`）
...
状态机在两种模式下都运行——无论是 `model_type=null`（快速模式）还是 `model_type="expert"`（专家模式），DeepSeek 都可能返回 THINK fragment。区别在于专家模式下 THINK fragment 几乎一定会出现，而快速模式下由模型自主决定。

### 7.8 SSE `reasoning_content` 字段
...
4. **流结束时发出 `{"finish_reason": "stop"}`**（§7.9）
...
| `model_type` | string/null | null | `"expert"` | **关键开关**，由 MODE 环境变量决定 |
...
| `search_enabled` | bool | false | 按需 | 启用联网搜索 |
| `preempt` | bool | false | false | 抢占模式，始终为 false |

**`model_type` 与 `thinking_enabled` 的协作：**

| 场景 | model_type | thinking_enabled | 说明 |
|------|-----------|-----------------|------|
| 纯快速模式 | `null` | `false` | 基础模式 |
| 快速+思考 | `null` | `true` | 快速响应但附带推理过程 |
| 专家无思考 | `"expert"` | `false` | 专家模型但不输出推理（罕见） |
| 完整专家 | `"expert"` | `true` | 专家模型 + 完整推理 |
...
| `dict/list` | 空标签（不支持嵌套） | `<parameter name="x"></parameter>` |

嵌套对象（dict/list 作为参数值）当前不被支持——会被序列化为空标签。这是一个已知限制。如果模型需要接收嵌套对象，需要扩展 `_format_params_dsml()` 使用 JSON.stringify + CDATA 方式传递。
...
**重要：** `MODE=auto` 不等于旧版行为。旧版中 `thinking_mode=true` 会同时设置 `model_type="expert"`。新版中 `model_type` 由 MODE 决定，`thinking_enabled` 由 THINKING 决定，两者完全独立。

#### MODE 与 THINKING 交互矩阵

| MODE | THINKING | 实际行为 |
|------|----------|---------|
| `auto` | `auto` | model_type=null, thinking=客户端决定 |
| `auto` | `enabled` | model_type=null, thinking=true（快速+思考） |
| `auto` | `disabled` | model_type=null, thinking=false |
| `quick` | `auto` | model_type=null, thinking=客户端决定 |
| `quick` | `enabled` | model_type=null, thinking=true |
| `quick` | `disabled` | model_type=null, thinking=false |
| `expert` | `auto` | model_type="expert", thinking=客户端决定 |
| `expert` | `enabled` | model_type="expert", thinking=true |
| `expert` | `disabled` | model_type="expert", thinking=false |
...
**工作原理：**
1. `netstat -ano` 列出所有连接及对应 PID
2. `findstr ":8080"` 过滤包含 8080 的行
3. `findstr LISTENING` 只保留 LISTENING 状态
4. `tokens=5` 提取第 5 列（PID）
5. `taskkill /F /PID` 强制终止
...
| 进程已退出但端口仍占用 | 前一个 uvicorn 未完全终止 | 用 `netstat -ano | findstr :8080` 找到 PID 后 taskkill |
...
> WASM 引擎版本：`sha3_wasm_bg.wasm`（DeepSeek 前端提取）
